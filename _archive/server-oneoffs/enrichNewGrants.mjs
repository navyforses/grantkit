/**
 * Targeted Grant Enrichment — Only enriches grants with empty applicationProcess field.
 * Used for newly imported grants that haven't been enriched yet.
 * 
 * Usage: node server/enrichNewGrants.mjs
 */
...
async function main() {
  console.log("=== GrantKit Targeted Enrichment (empty applicationProcess only) ===");
  
  // Fetch only grants that need enrichment
  const [rows] = await db.execute(
    sql`SELECT id, itemId, name, organization, description, eligibility, category, country, grantType, status, website, phone, grantEmail, amount, state FROM grants WHERE isActive = 1 AND (applicationProcess IS NULL OR applicationProcess = '') ORDER BY id ASC`
  );
  
  const allGrants = rows;
  console.log(`Found ${allGrants.length} grants needing enrichment\n`);
    ...
    const results = await Promise.allSettled(
      batch.map(async (grant) => {
        try {
          ...
          if (grant.state) existingInfo.push(`State/Region: ${grant.state}`);
...
Please research "${grant.organization || grant.name}" and provide all available information.`;
          ...
          if (setClauses.length > 0) {
            ...
            console.log(`  ✓ ${grant.name} (${setClauses.length} fields)`);
            return { status: "enriched", fields: setClauses.length };
          } else {
            console.log(`  ○ ${grant.name} (no updates)`);
  ...
  console.log(`Total processed: ${allGrants.length}`);
