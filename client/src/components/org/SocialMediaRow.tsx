/*
 * SocialMediaRow — row of chip-sized links for the org's social
 * profiles. Each chip renders a brand-accented icon and the platform
 * name; the whole chip is a clickable external link. Platforms without
 * a URL are skipped, so the row collapses cleanly when the `socialMedia`
 * JSON is partially populated.
 *
 * Icons imported from lucide-react for consistency; we use the
 * brand-neutral icons rather than the true brand colours so the row
 * sits well in both light and dark themes.
 */
...
export default function SocialMediaRow({ links }: Props) {
  ...
  const entries = SOCIAL_PLATFORMS.filter((key) => typeof links[key] === "string" && links[key]!.trim().length > 0);
