      ...
      <ul className="space-y-2">
        {result.criteria.map((c) => (
          <li key={c.key} className="flex items-start gap-2 text-sm">
            {c.status === "match" && (
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" aria-label="match" />
            )}
            {c.status === "no_match" && (
              <XCircle className="w-4 h-4 text-red-400 shrink-0 mt-0.5" aria-label="no match" />
            )}
            {c.status === "unknown" && (
              <HelpCircle className="w-4 h-4 text-muted-foreground shrink-0 mt-0.5" aria-label="unknown" />
            )}
            <div className="min-w-0 flex-1">
              <span className="text-muted-foreground/90">{c.label}:</span>{" "}
              {c.status === "unknown" ? (
                <Link
                  href={profileHref}
                ...
                <span className={c.status === "match" ? "text-foreground" : "text-foreground/80"}>
                  {c.value}
                </span>
              )}
            </div>
