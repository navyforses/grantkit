      ...
      <main className="flex-1 py-6 md:py-16 pb-24 md:pb-16">
        <div className="container px-4 md:px-0 max-w-3xl">
          ...
          <div className="prose prose-sm md:prose-base prose-gray max-w-none prose-headings:text-foreground prose-headings:tracking-tight prose-p:text-muted-foreground prose-p:leading-relaxed prose-li:text-muted-foreground prose-a:text-primary prose-a:no-underline hover:prose-a:underline prose-headings:text-base md:prose-headings:text-xl prose-p:text-sm md:prose-p:text-base prose-li:text-sm md:prose-li:text-base">
            ...
            <p>{t.legal?.termsAcceptText || "By accessing and using GrantKit (the \"Service\"), you agree to be bound by these Terms of Service. If you do not agree to these terms, please do not use our Service."}</p>

            <h2>{t.legal?.termsDescTitle || "Service Description"}</h2>
            <p>{t.legal?.termsDescText || "GrantKit provides a curated database of grants, foundations, and support resources for individuals, families, and caregivers worldwide. Our database is organized by category, country, and eligibility criteria, and is updated on a regular basis."}</p>

            <h2>{t.legal?.termsAccountTitle || "Account Registration"}</h2>
            <p>{t.legal?.termsAccountText || "To access certain features of our Service, you must create an account. You are responsible for maintaining the confidentiality of your account credentials and for all activities that occur under your account. You agree to provide accurate and complete information during registration."}</p>

            <h2>{t.legal?.termsSubscriptionTitle || "Subscription and Payment"}</h2>
            <p>{t.legal?.termsSubscriptionText || "Access to the full GrantKit database requires a paid subscription. Payments are processed by Paddle (paddle.com), our Merchant of Record. By subscribing, you agree to Paddle's terms of service and authorize recurring charges at the stated subscription rate."}</p>
            <ul>
            ...
            <p>{t.legal?.termsIPText || "All content on GrantKit, including but not limited to the database, text, graphics, logos, and software, is the property of GrantKit or its licensors and is protected by intellectual property laws. Your subscription grants you a personal, non-transferable license to access and use the content for personal, non-commercial purposes."}</p>

            <h2>{t.legal?.termsDisclaimerTitle || "Disclaimer"}</h2>
            <p>{t.legal?.termsDisclaimerText || "The information provided through GrantKit is for informational purposes only. While we strive to keep our database accurate and up-to-date, we make no warranties or representations about the completeness, accuracy, or reliability of the information. Grant availability, eligibility criteria, and deadlines may change without notice. We recommend verifying all information directly with the grant provider before applying."}</p>

            <h2>{t.legal?.termsLiabilityTitle || "Limitation of Liability"}</h2>
            <p>{t.legal?.termsLiabilityText || "To the maximum extent permitted by law, GrantKit shall not be liable for any indirect, incidental, special, consequential, or punitive damages arising from your use of the Service. Our total liability shall not exceed the amount you paid for the Service in the twelve months preceding the claim."}</p>

            <h2>{t.legal?.termsTerminationTitle || "Termination"}</h2>
            <p>{t.legal?.termsTerminationText || "We reserve the right to suspend or terminate your account if you violate these Terms of Service. Upon termination, your right to access the Service will immediately cease. You may also terminate your account at any time by cancelling your subscription and contacting us."}</p>

            <h2>{t.legal?.termsChangesTitle || "Changes to Terms"}</h2>
            <p>{t.legal?.termsChangesText || "We may modify these Terms of Service at any time. We will notify users of material changes by posting the updated terms on this page. Your continued use of the Service after changes constitutes acceptance of the modified terms."}</p>

            <h2>{t.legal?.termsGoverningTitle || "Governing Law"}</h2>
            <p>{t.legal?.termsGoverningText || "These Terms of Service shall be governed by and construed in accordance with the laws of Georgia, without regard to its conflict of law provisions."}</p>
