          ...
          <div className="space-y-6">
            <div className="bg-card rounded-xl border border-border shadow-sm p-5">
              <h2 className="text-base font-semibold text-foreground mb-4 flex items-center gap-2">
                ...
                {(a as any).tabExternal || "External Grant Search"}
              </h2>
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder={(a as any).extSearchPlaceholder || "Search 84,000+ grants..."}
                    value={extQuery}
                ...
                >
                  <option value="all">{(a as any).extSourceAll || "All Sources"}</option>
                  <option value="federal">{(a as any).extSourceFederal || "Federal"}</option>
                  <option value="foundation">{(a as any).extSourceFoundation || "Foundation"}</option>
                  <option value="state">{(a as any).extSourceState || "State"}</option>
                  <option value="international">{(a as any).extSourceIntl || "International"}</option>
                </select>
                <button
              ...
              <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                ...
                {extLoading ? (
                  <div className="p-12 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">{(a as any).extSearching || "Searching external databases..."}</p>
                  </div>
                  ...
                  <div className="divide-y divide-border">
                    {extResults.results.map((grant) => (
                      <div
                        key={grant.slug}
                        className={`px-5 py-4 hover:bg-secondary/30 transition-colors cursor-pointer ${extSelectedSlug === grant.slug ? "bg-primary/5 border-l-2 border-l-primary" : ""}`}
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="text-right shrink-0">
                            {grant.amount && <span className="text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">{grant.amount}</span>}
                            {grant.deadline && grant.deadline !== "No deadline" && <p className="text-xs text-muted-foreground mt-1">{grant.deadline}</p>}
                          </div>
                        </div>
                        {grant.confidence && <span className="inline-flex items-center mt-2 text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{grant.confidence}</span>}
                      </div>
                    ))}
                  </div>
                )}
                ...
                {extDetailLoading ? (
                  ...
                  <div>
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <button onClick={() => setExtSelectedSlug(null)} className="text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button>
                    </div>
                    ...
                    {extDetail.detail.applyUrl && (
                      <a href={extDetail.detail.applyUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm text-primary hover:underline mb-4">
                        <ExternalLink className="w-3.5 h-3.5" /> {a.formWebsite}
                      </a>
                    )}
                    {extDetail.mappedGrant && (
                      <div className="mt-4 p-3 bg-primary/5 rounded-lg border border-primary/10">
                        <p className="text-xs font-medium text-primary mb-2">{(a as any).extMappedCategory || "Auto-mapped category"}:</p>
                        <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{categoryLabels[extDetail.mappedGrant.category] || extDetail.mappedGrant.category}</span>
                      </div>
                    )}
                    <div className="flex gap-3 mt-6 pt-4 border-t border-border">
                      <button
                        onClick={() => { if (!extDetail.mappedGrant) return; setExtImporting(true); importExternalMutation.mutate({ ...extDetail.mappedGrant, notifySubscribers: false }); }}
                      ...
                      >
                        {extImporting ? <Loader2 className="w-4 h-4 animate-spin" /> : <PlusCircle className="w-4 h-4" />}
                        {(a as any).extAddToCatalog || "Add to Catalog"}
                      </button>
                      <button
                        onClick={() => { if (!extDetail.mappedGrant) return; setExtImporting(true); importExternalMutation.mutate({ ...extDetail.mappedGrant, notifySubscribers: true }); }}
                        disabled={extImporting || importExternalMutation.isPending}
                        className="px-6 py-2.5 text-sm font-medium text-white bg-primary hover:bg-[#162d4a] rounded-lg transition-colors disabled:opacity-50 inline-flex items-center gap-2"
                      >
                        <Bell className="w-4 h-4" />
                        {(a as any).extAddAndNotify || "Add & Notify Subscribers"}
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">{(a as any).extDetailNotFound || "Could not load grant details"}</p>
                  </div>
                )}
              </div>
            )}
